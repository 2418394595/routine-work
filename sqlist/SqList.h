...
template<>
void SqList<stu>::CreateList(int n)
{
	if (n>listsize) throw"参数非法";
	cout << "输入格式为：学号 姓名 成绩" << endl;
	cout << "请依次输入" << n << "个元素值：" << endl;
	for (int i = 1; i <= n; i++)
	{
		cin >> elem[i - 1].xh;
...
template<>
int SqList<stu>::Locate(stu e)
{
	for (int i = 0; i < length; i++)
	{
		if (elem[i].xh == e.xh&&!strcmp(elem[i].xm,e.xm)&&elem[i].cj == e.cj)
		{
			return i + 1;
		}
	}
